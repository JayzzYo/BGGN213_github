---
title: "Class19_Pertussis_Project"
...
> Q1. Make a plot of cases per year
...
> Q2. Add some annotation for some major milestones in our interaction with Pertussis. The origional wP vaccinedeployment in 1947 and the newer aP vaccine roll-out in 1996. Finally, a line in 2020.
...
They make their data availble via a JSON format returning API. We can read JSON format with the `read_json` function from the **jsonlite** package.
...
> Q5. What is the `biological_sex` and `race` breakdown of the subjects?
...
Let's read more tables from the CMI-PB API
...
Join (or link or merge) using the `inner_join()` function from the **dplyr**.
...
igg |>
  filter(antigen == "PT") |> 
  filter(dataset == "2021_dataset") -> PT
```
